...
if (process.env.CLIENT_URL) {
  // Allow multiple origins if CLIENT_URL contains commas
  const extra = process.env.CLIENT_URL.split(',').map(s => s.trim());
...
app.get('/*', (req, res) => {   // ← note the '/*' instead of '*'
  const publicDir = path.join(__dirname, 'public');
...
app.listen(PORT, () => {
  console.log('\n🚀 FileMaster Pro Backend STARTED (MongoDB Atlas)');
